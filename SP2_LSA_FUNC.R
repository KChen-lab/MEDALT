...
shuffleCNV <- function(j, CNV){
    sub_samps <- grep(paste(j,"_",sep=""), colnames(CNV))
    subCNV <- CNV[,sub_samps]
    if(length(sub_samps)==1){
        subCNV <- as.data.frame(subCNV)}
...
permuteSeg <- function(CNV, generegion){
    ...
    chr=unique(chr[,1])
    CNV1=do.call(cbind,lapply(chr, shuffleCNV, CNV))
...
###  celltree: real tree                                                                    ###
###  cell_pem: cell_list for permutation                                                    ###
###############################################################################################
find_NAs <- function(x){ return(ifelse(NA %in% x, 0, 1))}

permuteScore <- function(data, ID, ans, datatype, pathwaygene, generegion, reference, celltree){
    if (datatype == "D"){
        CNVpermuBand <- permuteSeg(data, generegion)     # DNA label permuted
        CNVpermuCell <- permuteID(ID, CNVpermuBand, ans) # cell name permuted
        CNVpermuGene <- do.call(cbind, lapply(1:dim(pathwaygene)[1], geneCNAfunction, # function
                                         pathwaygene, CNVpermuBand, generegion)) # arguments
        colnames(CNVpermuGene) <- pathwaygene$name
        rownames(CNVpermuGene) <- rownames(CNVpermuBand)
        index=apply(CNVpermuGene,2,find_NAs)
        CNVpermuGene=CNVpermuGene[,index==1]
    }
    if (datatype == "R"){
        CNVpermuBand <- permuteGene(data, reference)
        index <- match(colnames(CNVpermuBand), rownames(generegion))
        CNVpermuCell <- CNVpermuBand
        colnames(CNVpermuCell) <- generegion$ID1[index]
        CNVpermuCell <- permuteID(ID,CNVpermuCell,ans)
        index <- match(as.character(pathwaygene$name),colnames(CNVpermuBand))
        CNVpermuGene <- CNVpermuBand[,index[!is.na(index)]]
    }
    bandGscore <- lapply(cell_pem$cells, lineageScore, CNVpermuCell, celltree)
    names(bandGscore) <- cell_pem$cells
    geneGscore <- lapply(cell_pem$cells, lineageScore, CNVpermuGene, celltree)
    names(geneGscore) <- cell_pem$cells
    return(list(bandGscore=bandGscore,
                geneGscore=geneGscore))}
...
permuteTreeScore <- function(permutetree,ID,ans,datatype,pathwaygene,generegion,reference,permutationPath){
  ...
  permutecell$child=sapply(as.character(permutecell$cell),child,cellTree=permutecelltree)
  permutecell1=permutecell[permutecell$child>=5,]
...
significanceLevel <- function(x,realband,realgene,permuteres,realcell){
  sizecutoff=realcell$child[realcell$cell==as.character(realcell$cell)[x]]
  if ("cell" %in% names(permuteres[[1]])){
    permutedis=lapply(1:length(permuteres),function(j,permuteres,cutoff){
      ...
      delt=abs(permutecell$child-cutoff)
...
### type: D or R                                                                            ###
### realcell: all cells from real tree.                                                     ###
### A dataframe includes cellID, corresponding depth in the tree and the number of children ###
###############################################################################################
GenePara <- function(band_sig, permuteres, type, realcell){
    run_p_value <- function(x, paraGene, band_sig){ ### 1st helper used in the lapply in main function
        region  <- names(paraGene)[x]                 # band that in multiple cluster
        subres  <- band_sig[band_sig$region==region,] # sub_results of this band
        CNAsign <- sign(subres$Score)
        CNA_idx <- 0
        if (sum(CNAsign==( 1))>1){ subsize=subres$child[CNAsign==( 1)] ; CNA_idx=1 } # examine amp CNAs
        if (sum(CNAsign==(-1))>1){ subsize=subres$child[CNAsign==(-1)] ; CNA_idx=2 } # examine del CNAs
        if (CNA_idx!=0){
            permutelineage <- lapply(1:length(permuteres), run_permute_lineage, permuteres,subsize,realcell,CNA_idx,region)
            permutelineage <- sapply(permutelineage, function(s){return(sum(s))})
            pvalue=(sum(permutelineage>=length(subsize))+1)/(length(permutelineage)+1)
            return(pvalue)
        }else{return(NA)}}

    run_permute_lineage <- function(i, permuteres, subsize, realcell, CNA_idx, region){### 2nd helper in the lapply of run_p_value
        format <- "cell" %in% names(permuteres[[i]])    # maybe in some format, 
        if (format){permutecell <- permuteres[[i]]$cell # permuters has a "cell"?
       }else{       permutecell <- realcell }
        if (type=="gene"){ permuteScore <- permuteres[[i]]$geneGscore }
        if (type=="band"){ permuteScore <- permuteres[[i]]$bandGscore }
        permutelineage <- c()
        for (s in 1:length(subsize)){
            Delt <- abs(permutecell$child-subsize[s])
            MinD <- which(Delt==min(Delt))
            scor <- c()
            if (CNA_idx==1){
                for (j in MinD){tdf<-permuteScore[[j]] ; scor<-c(scor, max(tdf[CNA_idx,grep(region,colnames(tdf))]))}
                permutelineage = c(permutelineage, sum(scor>=( 0.5)))}
            if (CNA_idx==2){
                for (j in MinD){tdf<-permuteScore[[j]] ; scor<-c(scor, min(tdf[CNA_idx,grep(region,colnames(tdf))]))}
                permutelineage = c(permutelineage, sum(scor<=(-0.5)))}}
        return(permutelineage)}
    
    ### main function body. Unfortunately, due to the messy rules of variables in global/local scope
    ### I don't have much time to optimize the whole program.
    paraGene <- table(as.character(band_sig$region))  # all significant bands
    paraGene <- paraGene[paraGene>1]                  # sig bands that show up in multiple clusters
    paraPval <- unlist(lapply( 1:length(paraGene), run_p_value, paraGene, band_sig)) ### run_p_value
    paraGene <- data.frame(region=names(paraGene), lineage=as.numeric(paraGene), pvalue=paraPval)
    return(paraGene)}
...
mergeCNA <- function(node,sigCNA,band.region,arm.band,refer.band){
    refineSigregion <- function(i,psub,type,refer.band,arm.band,subres,subregion){
      if (dim(psub)[1]==1){
        ...
        if (f > 0.5){
          subres=rbind(subres,data.frame(region=paste(i,type,sep=":"),Score=mean(psub$Score),pvalue=mean(psub$pvalue),
                                         adjustp=mean(psub$adjustp),cell=unique(as.character(psub$cell)),depth=unique(psub$depth)))
        }else{
          subband=subregion[subregion[,1]==i,]
          subband=do.call(rbind,strsplit(subband[,2],split="[.]"))
          subband=unique(subband[,1])
            
            func_1 <- function(x,psub){
            sub1=psub[grep(x,as.character(psub$region)),]
            if (dim(sub1)[1]==1){ return(sub1) }else{
              ...
              if (f > 0.5){
                return(data.frame(region=paste(i,x,sep=":"),Score=mean(sub1$Score),pvalue=mean(sub1$pvalue),
                                  adjustp=mean(sub1$adjustp),cell=unique(as.character(sub1$cell)),depth=unique(sub1$depth)))
              }else{ return(sub1) }}}
            
          sub1=do.call(rbind,lapply(subband,func_1,psub))
...
###########################################################################
# define a function to estimate in/del score
score_indel <- function(x){
    Gamp <- ifelse(sum(x>2)==0, 0, sum(x[x>2]-2)/length(x)) 
    Gdel <- ifelse(sum(x<2)==0, 0, sum(x[x<2]-2)/length(x)) 
    return(c(Gamp, Gdel))}

lineageScore <- function(node, newCNV, celltree){
    # get sub_branch from the whole tree
    child <- splitTree(node, celltree)
    index <- match(child,row.names(newCNV))
    subcnv<- newCNV[index,]
    # and finally apply it
    Gscore=apply(subcnv, 2, score_indel)
    return(Gscore)}
      ...
      while (k < dim(subsig1)[1]){
        if (subsig1$band[k+1]-subsig1$band[k]==1&(subsig1$Score[k+1]*subsig1$Score[k])>0&subsig1$arm[k+1]==subsig1$arm[k]){
          ...
          if (k == dim(subsig1)[1]){
            ...
            tempsig=data.frame(region=region,Score=mean(subsig1$Score[start:end]),pvalue=mean(subsig1$pvalue[start:end]),adjustp=mean(subsig1$adjustp[start:end]),cell=as.character(subsig1$cell)[start],depth=subsig1$depth[start],child=subsig1$child[start])
        ...
        while (k < dim(subsig22)[1]){
          if (subsig22$chr[k+1]==subsig22$chr[k]&subsig22$arm[k+1]==subsig22$arm[k]&(subsig22$Score[k+1]*subsig22$Score[k])>0&subsig22$band[k+1]-subsig22$band[k]==1){
            ...
            if (k == dim(subsig22)[1]){
              ...
              tempsig=data.frame(region=region,Score=mean(subsig22$Score[start:end]),pvalue=mean(subsig22$pvalue[start:end]),adjustp=subsig22$adjustp[start:end],cell=as.character(subsig22$cell)[start],depth=subsig22$depth[start],child=subsig22$child[start])
...
### get chr arm position
get_arm_position = function(x, sub.band, sub__ref){
    mappd = which(sub.band[,1]==x)
    start = sub__ref[min(mappd),2]
    endin = sub__ref[max(mappd),3]
    return(c(start, endin))}


### get CNV_band from CNV_gene
subsetCNV <- function(id, CNV_gene, bed_outs, gene_loc){
    chrID <- bed_outs$ID1[bed_outs$ID==id]
    index <- match(chrID, gene_loc$ID1)
    return(round(apply(CNV_gene[index,3: dim(CNV_gene)[2]], 2, mean)))}

### calculate de novo permutation score
denovo_perm_score <- function(j, CNV_gene, ID, bed_outs, datatype, onc_pth, generegion, ref_gene, sct_edges){
    if (j%%rpfeq==0){ 
        time_run <- Sys.time()-tt
        print.noquote(paste("iter ", j, ", avg.time: ", round((time_run)/j*rpfeq,3), " eta: ", round(time_run/j*(times-j),3), sep=""))
        tt <- Sys.time()}
    premte_score <- permuteScore(CNV_gene, ID, bed_outs, datatype, onc_pth, gene_loc, ref_gene, sct_edges)
    return(premte_score)}